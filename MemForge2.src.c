/*
 * MemForge2 v0.4.44 — UEFI memory tester written from scratch.
 *
...
/* v0.4.44 — soft deadline flag. The BSP sets this when a timed kernel
...
/* v0.4.44 — hardware watchdog (seconds). The BSP re-arms ("kicks") this at
...
static void init_splash(CHAR16 *stage) {
    ...
    CHAR16 *title = L"MEMFORGE v0.4.44";
...
/* v0.4.44 — focused cards layout for small screens (g_h < 900).
...
static void compute_layout(UINTN n_tests) {
    ...
    /* v0.4.44 — focused layout on small screens.
...
static void render_header(UINT64 elapsed_ms, UINTN done, UINTN total) {
    ...
    if (cols >= 110) {
        SPrint(buf, sizeof(buf),
               T(L"  MEMFORGE v0.4.44   |   %ld.%ld ГБ RAM   |   %s   "
                 L"|   %s   |   прошло %02d:%02d   |   осталось ~%02d:%02d   |   Тесты %d/%d",
                 L"  MEMFORGE v0.4.44   |   %ld.%ld GB RAM   |   %s   "
        ...
        SPrint(buf, sizeof(buf),
               T(L"  MEMFORGE v0.4.44   |   %ld.%ld ГБ RAM   |   %s   |   %s   |   прошло %02d:%02d   |   осталось ~%02d:%02d",
                 L"  MEMFORGE v0.4.44   |   %ld.%ld GB RAM   |   %s   |   %s   |   elapsed %02d:%02d   |   ETA ~%02d:%02d"),
               ram_gb_x10 / 10, ram_gb_x10 % 10,
               pass_tag,
               err_tag,
               secs / 60, secs % 60,
        ...
        SPrint(buf, sizeof(buf),
               T(L"  MEMFORGE v0.4.44  |  %ld.%ld ГБ RAM  |  %s  |  %s  |  прошло %02d:%02d",
                 L"  MEMFORGE v0.4.44  |  %ld.%ld GB RAM  |  %s  |  %s  |  elapsed %02d:%02d"),
               ram_gb_x10 / 10, ram_gb_x10 % 10,
               pass_tag,
               err_tag,
               secs / 60, secs % 60);
        ...
        SPrint(buf, sizeof(buf),
               T(L" MEMFORGE v0.4.44 | %s | %s | прошло %02d:%02d",
                 L" MEMFORGE v0.4.44 | %s | %s | elapsed %02d:%02d"),
               pass_tag,
               err_tag,
               secs / 60, secs % 60);
    }
...
/* v0.4.44 — per-DIMM error tally across the ENTIRE run. g_err_records caps at
...
static void record_error(kernel_id_t test, UINT32 core,
    ...
    /* v0.4.44 — tally per DIMM for EVERY error (not only the 32 stored below).
...
static int dominant_dimm_idx(void) {
    /* v0.4.44 — pick the DIMM with the most errors across the WHOLE run.
...
/* v0.4.44 — detect dual-channel interleave ambiguity.
   ...
   Pre-v0.4.44 verdict confidently said "REPLACE: DDR4-B2 (HIGH)" — but
...
static UINTN distributed_dimm_indices(int *out_idx, UINTN cap) {
    /* v0.4.44 — use the true per-DIMM totals (every error), not the first 32. */
...
/* v0.4.44 — Approach D: detect whether SMBIOS Type 20 reports REAL
...
/* v0.4.44 — Approach A: bit-6 polarity analysis of error addresses.
...
/* v0.4.44 — count APs that took the legacy PERF_CTL (0x199) path
...
static UINT32 try_enable_max_perf(void) {
    ...
    if (turbo_ratio > 0) {
        ...
        /* v0.4.44 — bump the legacy-turbo counter so the per-run [PERF]
...
typedef struct {
    ...
    /* v0.4.44 — socket-vs-stick tracking. Repurposed from reserved[]; an old
...
static void hist_save_and_diff(UINT64 total_ms) {
    ...
    /* v0.4.44 — capture WHICH slot dominated this run's errors and WHICH stick
    ...
    if (g_hist_prev_valid) {
        ...
        /* v0.4.44 — socket-vs-stick differential. SAME slot tops the errors
...
static void amd_thermal_probe(void) {
    ...
    for (UINT8 dev = 0; dev < 32; dev++) {
        ...
        /* v0.4.44 — use the CORRECT decode (same as amd_thermal_sample):
           apply 0x7FF mask AND bit-19 -49°C range adjust. Pre-v0.4.44
...
static UINT32 amd_thermal_sample(void) {
    /* v0.4.44 — correct decode per Linux k10temp / FreeBSD amdtemp.c:
       ...
       Pre-v0.4.44 code was missing both the 0x7FF mask AND the bit-19
...
/* v0.4.44 — quick-test per-DIMM targeting. Quick mode used to test 3 consecutive
...
/* v0.4.44 — map a kernel enum (KER_*) to its position in g_tests[].
...
/* v0.4.44 — Forward decls for focused-mode helpers (defined below
...
/* ---------- Focused-mode card painters (v0.4.44) ---------- */
...
static void card_focused_paint(UINTN i) {
    ...
    /* Row 1: test name (left) + short description in dim color + index counter (right).
       v0.4.44 — description lets non-expert user know what the test
...
static void core_cols_compute(core_cols_t *c) {
    ...
    /* v0.4.44 — "Смещ" (buffer-offset for this core's slice) column dropped
    ...
    (void)w_addr;
    /* v0.4.44 — the activity bar duplicates the "99%" number right next to it;
...
static void core_panel_update(void) {
    ...
    for (UINTN i = 0; i < shown; i++) {
        ...
        /* Bar — short colour cue in its own narrow column; the "99%" number
           carries the precise value. v0.4.44 — thin, fixed ~8 px in BOTH modes
...
volatile int g_force_kernel_exit = 0;   /* v0.4.44 — BSP-forced timed-kernel deadline */
/* v0.4.44 — anti-hang watchdog state. A worker core that never reports "done"
...
/* v0.4.44 — countdown UX rework.
   Pre-v0.4.44: ESC meant "skip the wait and start the test now" — which
...
static void bsp_yield_render(ap_arg_t *a) {
    ...
    g_last_yield_ms = now;
    watchdog_kick();   /* v0.4.44 — BSP alive → keep the reboot watchdog at bay */
...
static test_summary_t run_test_mc(UINTN test_idx) {
    ...
    g_last_yield_ms    = 0;
    g_force_kernel_exit = 0;   /* v0.4.44 — clear deadline flag for this test */
    watchdog_kick();           /* v0.4.44 — arm the hardware reboot watchdog */
    ...
    if (g_mp && g_n_enabled > 1) {
        ...
        for (UINTN i = 1; i < g_n_enabled; i++) {
            if (g_core_dead[i]) continue;   /* v0.4.44 — skip cores that stalled earlier */
    ...
    /* Once-per-run diagnostic: how many APs successfully bumped the
       CPU into max P-state, by which mechanism. Pre-v0.4.44 this only
    ...
    /* Spin-poll worker (AP) done flags. Also polls the keyboard so ESC during
       ...
       v0.4.44 — anti-hang watchdog for EVERY kernel, not just the two timed
       ...
       at duration + 60 s — identical to the old v0.4.44 behaviour. For the
    ...
    if (g_mp && g_n_enabled > 1) {
        for (UINTN spin = 0; spin < 36000; spin++) {
            ...
            watchdog_kick();   /* v0.4.44 — BSP alive while waiting on workers */
    ...
    watchdog_off();   /* v0.4.44 — test done; no reboot at menu/verdict/report */
    return s;
}
...
static verdict_kind_t compute_verdict_kind(void) {
    /* v0.4.44 — errors take priority over an abort. If the user stopped the
    ...
    if (g_core_stall_count>0) return VERDICT_WARN;   /* v0.4.44 — incomplete run */
...
/* ---------- v0.4.44 Auto-isolation feature ----------
...
/* v0.4.44 — should auto-isolation kick in automatically?
...
static void render_simple_verdict(UINT64 total_ms) {
    ...
    /* v0.4.44 — reset isolation offer; will be enabled below if applicable. */
    g_iso_offer = 0;
    g_iso_dimm_n = 0;
    ...
    /* v0.4.44 — if the user stopped the run early, flag it right under the
    ...
    /* v0.4.44 — when there are errors, show the TRUE per-DIMM split right here
    ...
    if (v == VERDICT_PASS) {
        ...
        /* v0.4.44 — Approach D + A: classify WHY errors are distributed.
        ...
        if (dist_kind == DIST_BOTH_STICKS_BAD) {
            ...
            gfx_draw_str_color(cx, cy,
                ...
                COL_DIM); cy += cline + 6;
            /* v0.4.44 — offer auto-isolation: re-test each DIMM in its own
               ...
               v0.4.44 — only offer [I] if auto-isolation hasn't already
    ...
    /* Footer hint — same key handling as the technical summary, plus [D].
       v0.4.44: [I] for auto-isolation when offered. */
    UINTN foot_y = g_h - g_char_h - 8;
    blt_fill(0, foot_y - 4, g_w, g_char_h + 8, COL_PANEL_ALT);
    blt_fill(0, foot_y - 5, g_w, 1, COL_BORDER);
    CHAR16 footer[260];
...
static void render_summary(UINT64 total_ms) {
    ...
    CHAR16 buf[200];
    SPrint(buf, sizeof(buf),
           T(L"  MEMFORGE v0.4.44 ИТОГИ   |   %d сек   |   Ядра %d/%d",
             L"  MEMFORGE v0.4.44 SUMMARY   |   %d sec   |   Cores %d/%d"),
    ...
    if (g_err_count > 0) {
        ...
        if (stuck_n >= 5 && stuck_x != 0) {
            ...
            if (bp >= 0) {
                ...
                /* v0.4.44 — use SMBIOS Type 17 locator string ("DDR4-B2")
        ...
        /* (3) 1-GB histogram — v0.4.44: short label on its own row, then
...
static void write_json_report(UINT64 total_ms) {
    ...
    SPrint(buf, sizeof(buf),
        ...
        (grand_err > 0 ? "FAIL"                      /* v0.4.44 — errors win, */
            : (g_aborted ? "ABORTED"                 /* even if user aborted   */
            : (g_core_stall_count > 0 ? "INCOMPLETE" : "PASS"))));
    json_write_chunk(jf, buf);

    /* v0.4.44 — surface core stalls so the analyzer never reads a stalled
       (incomplete) run as a clean PASS. */
    SPrint(buf, sizeof(buf), L"  \"core_stalls\": %d,\r\n", g_core_stall_count);
    json_write_chunk(jf, buf);

    /* v0.4.44 — per-DIMM error totals across the whole run (every error, not
    ...
    /* v0.4.44 — run-wide peaks. Lets an automated analyzer verify the
...
/* v0.4.44 — log the geometry the renderer ACTUALLY uses at a given moment, plus
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    if (g_gop) {
        ...
        /* v0.4.44 — robust mode picker with per-mode verification.
           ...
           was at 3440x1440. The pre-v0.4.44 LATE check eventually caught
        ...
        for (UINTN i = 0; i < n_cand; i++) {
            ...
            /* v0.4.44 — verify against THIS mode's OWN required framebuffer
               ...
               after a silently-failed SetMode, which made v0.4.44 reject
    ...
    log_line(L"=== MemForge2 v0.4.44 init ===");
...

    {
        ...
        if (g_gop) {
            ...
            for (UINT32 m = 0; m < n_modes && m < 32; m++) {
                ...
                if (uefi_call_wrapper(g_gop->QueryMode, 4,
                    ...
                    continue;
                /* v0.4.44 — also log PixelFormat and PixelsPerScanLine
        ...
        /* v0.4.44 — MP Services Protocol diagnostic. Without this log it
    ...
    while (!reboot_requested) {
        ...
        g_core_stall_count = 0;   /* v0.4.44 — clear stall tally for fresh run */
        g_err_count = 0;          /* v0.4.44 — fresh error tally each run */
        for (UINTN d = 0; d < MAX_DIMMS; d++) g_dimm_err_count[d] = 0;
        for (UINTN i = 0; i < g_n_enabled; i++) {
            g_args[i].throttle_event_count = 0;
            g_args[i].throttle_was_active  = 0;
            g_core_dead[i] = 0;   /* v0.4.44 — un-blacklist any stalled cores */
        }
        ...
        if (g_quick_mode) {
            /* v0.4.44 — quick triage now samples EVERY DIMM (one chunk per
        ...
        for (UINT32 pass = 0; pass < passes_target && !g_aborted; pass++) {
            ...
            if (g_cfg_multipass) {
                ...
                int alloced = 0;
                /* v0.4.44 — quick mode: allocate at this pass's per-DIMM target
        ...
        for (UINTN i = 0; i < N_TESTS && !g_aborted; i++) {
            ...
            /* v0.4.44 — countdown returns 0=start, 1=skip this test, 2=abort run */
            ...
            /* v0.4.44 — clear the countdown footer once the test starts.
            ...
            /* Flush after each test completes — user might yank USB at any
               ...
               1× per log line). */
            flush_log_now();
            /* v0.4.44 — ACCUMULATE across marathon passes, do not OVERWRITE.
               Pre-v0.4.44 the line was `g_summary[i] = r;` which kept only
        ...
        /* v0.4.44 — auto-isolation: if errors are distributed across 2+
        ...
        while (!leave_summary) {
            ...
            if (k.ScanCode == SCAN_ESC) {
                ...
                leave_summary = 1;        /* fall through → main menu */
                       ...
                       && g_iso_offer) {
                /* v0.4.44 — auto-isolation: re-test each affected DIMM with
